...
class State:
    ...
    def __init__(self, newState):
        ...
        for i in range(0,8): #for each tile
...
def rbfs(prob, node, fLim, pList): #handles the rbfs
    ...
    if len(successors) == 0: #if there are no successors
        return None, math.inf, pList #return no node, infinite fcost, and current path list
    for su in successors: #for each successor in the list
        su.f = max(su.f, node.f) #determine if the successor fcost is higher than that of the current node
    while True: #repeat forever
        ...
        if best.f > fLim: #if the fcost of the best node is greater than that of the flim
            return None, best.f #return no node, but maintain flim
        ...
        result, best.f, pListNew = rbfs(prob, best, min(fLim, alternative), pList) #begin to expand the best successor
        if result is not None: #if there is a node returned
            pListNew.insert(0, node) #push the current node onto the path list
            return result, best.f, pListNew #return the current nodem, best fcost, and new path list
...
def main():
    ...
    puzzle = Problem()
    pathList = []
    result, best, pathList = rbfs(puzzle, x, math.inf, pathList)
    for t in pathList:
        t.printState()
